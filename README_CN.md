...
| API | 模型 | 端点 | 请求格式 |
|---|---|---|---|
| **Predict API** | `imagen-3.0-*` | 仅 Vertex AI | `instances[].prompt` |
...
| 模型 ID | 质量 | 速度 | 价格 | 适用场景 |
|---|---|---|---|---|
| `imagen-3.0-generate-002` | **高** | 较慢 | ~$0.04/张 | 生产环境、营销素材 |
| `imagen-3.0-fast-generate-001` | 良好 | **快** | ~$0.02/张 | 快速迭代、原型设计 |
| `gemini-2.0-flash-preview-image-generation` | 良好 | 快 | 按量计费 | 多模态文图混合 |

> **关键提示**：每个模型有独立的 API 配额。如果 `imagen-3.0-generate-002` 遇到 429 错误，切换到 `imagen-3.0-fast-generate-001` 可以正常使用，因为它们的速率限制是分开的。
