...
- **多模型** — Gemini 3.1 / 3 Pro Image、Gemini 2.5 Flash Image，以及 Vertex AI 上的旧版 Imagen 4
...
| API | 模型 | 端点 | 请求格式 |
|---|---|---|---|
| **Predict API** | `imagen-4.0-*` | 仅 Vertex AI | `instances[].prompt` |
...
使用 GCP 计费 + Gemini 图片模型、可选旧版 Imagen 4，并启用图片编辑和超分能力。
...
```
generate_image(prompt="一张日落风景图", model="gemini-3-pro-image")
```
...
```bash
# AI Studio（Gemini 模型）
GEMINI_MODEL=gemini-3.1-flash-image      # 默认，质量和速度平衡
GEMINI_MODEL=gemini-3-pro-image          # 更强推理和构图
GEMINI_MODEL=gemini-2.5-flash-image      # 备用

# Vertex AI（Gemini + 旧版 Imagen 模型）
GEMINI_MODEL=gemini-3.1-flash-image        # 默认
GEMINI_MODEL=imagen-4.0-generate-001       # 旧版 Imagen；2026-08-17 退役
```
...
| `GEMINI_MODEL` | 否 | `gemini-3.1-flash-image` | 默认模型（可通过 `model` 参数逐次覆盖） |
...
| 模型 ID | 质量 | 速度 | 价格 | 适用场景 |
|---|---|---|---|---|
| `gemini-3.1-flash-image` | **最高** | 快 | 免费 | 推荐默认 |
| `gemini-3-pro-image` | **最高** | 较慢 | 免费 | 强推理构图 |
| `gemini-2.5-flash-image` | 良好 | 快 | 免费 | 备用 |

### Vertex AI（Imagen）— GCP 赠金

| 模型 ID | 质量 | 速度 | 价格 | 适用场景 |
|---|---|---|---|---|
| `gemini-3.1-flash-image` | **最高** | 快 | 按量计费 | 推荐默认 |
| `gemini-3-pro-image` | **最高** | 较慢 | 按量计费 | 强推理构图 |
| `gemini-2.5-flash-image` | 良好 | 快 | 按量计费 | 备用 |
| `imagen-4.0-generate-001` | 高 | 快 | ~$0.04/张 | 旧版 Imagen；2026-08-17 退役 |
| `imagen-4.0-ultra-generate-001` | 最高 | 较慢 | ~$0.06/张 | 旧版 Imagen；2026-08-17 退役 |
| `imagen-4.0-fast-generate-001` | 高 | 最快 | ~$0.02/张 | 旧版 Imagen；2026-08-17 退役 |
...
3. 找到对应模型（如 `gemini-3.1-flash-image` 或 `imagen-4.0-generate`）
...
> **提示**：新配置优先使用 Gemini 图片模型。Imagen 4 生成模型仍保留给旧版 Vertex AI 配置，但计划在 2026-08-17 退役。
...
| `User location is not supported` | 模型有地区限制 | 换一个 `GCP_REGION` 或换成 `gemini-3.1-flash-image` |
